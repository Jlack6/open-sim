...
3. **Keep it generic.** A core principle of open-sim is *no hardcoded app logic*. Tools should expose generic primitives; app-specific knowledge belongs in `knowledge/` (local) or `test_cases/`, not in `src/`.
4. **Match the existing style.** Small, focused commits; clear tool descriptions; errors wrapped so a single failure never crashes the server.
